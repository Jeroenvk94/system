...
class Request
{
    ...
    {
        if ($this->method === null) {
            $requestMethod = strtoupper($this->server('REQUEST_METHOD', 'GET'));

            // POST + '_method' override to emulate REST behavior in browsers that do not support it
            if ($requestMethod == 'POST' && $this->get('_method')) {
    ...
    {
        return (bool) ((!isset($this->server('HTTPS')) || strtolower($this->server('HTTPS')) != 'on') ? false : true);
    }
    ...
    {
        return !isset($this->server('HTTP_HOST'));
    }
